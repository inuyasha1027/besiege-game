...
public final class Voronoi {
    ...
    private HashMap<PointH, Site> _sitesIndexedByLocation;
    ...
    public Voronoi(ArrayList<PointH> pointHs, ArrayList<Color> colors, Rectangle plotBounds) {
        _sites = new SiteList();
        _sitesIndexedByLocation = new HashMap();
        addSites(pointHs, colors);
    ...
    private void addSites(ArrayList<PointH> pointHs, ArrayList<Color> colors) {
        int length = pointHs.size();
        for (int i = 0; i < length; ++i) {
            addSite(pointHs.get(i), colors != null ? colors.get(i) : null, i);
        }
    }

    private void addSite(PointH p, Color color, int index) {
    ...
    public ArrayList<PointH> region(PointH p) {
    ...
    public ArrayList<PointH> neighborSitesForSite(PointH coord) {
        ...
        for (Site neighbor : sites) {
            pointHs.add(neighbor.get_coord());
        }
        return pointHs;
    }
    ...
    private ArrayList<Edge> selectEdgesForSitePoint(PointH coord, ArrayList<Edge> edgesToTest) {
    ...
    private ArrayList<LineSegment> visibleLineSegments(ArrayList<Edge> edges) {
        ...
        for (Edge edge : edges) {
            if (edge.get_visible()) {
                PointH p1 = edge.get_clippedEnds().get(LR.LEFT);
                PointH p2 = edge.get_clippedEnds().get(LR.RIGHT);
    ...
    public ArrayList<LineSegment> voronoiBoundaryForSite(PointH coord) {
        return visibleLineSegments(selectEdgesForSitePoint(coord, _edges));
    }

    public ArrayList<LineSegment> delaunayLinesForSite(PointH coord) {
    ...
    public ArrayList<PointH> hullPointsInOrder() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            pointHs.add(edge.site(orientation).get_coord());
        }
        return pointHs;
    }
    ...
    public ArrayList<ArrayList<PointH>> regions() {
    ...
    public ArrayList<PointH> siteCoords() {
    ...
    public static int compareByYThenX(Site s1, PointH s2) {
        if (s1.get_y() < s2.y) {
            return -1;
        }
        if (s1.get_y() > s2.y) {
