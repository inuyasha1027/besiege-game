...
public final class Edge {
    ...
    public double sitesDistance() {
        return PointH.distance(get_leftSite().get_coord(), get_rightSite().get_coord());
    }
    ...
    private HashMap<LR, PointH> _clippedVertices;

    public HashMap<LR, PointH> get_clippedEnds() {
    ...
    public void clipVertices(Rectangle bounds) {
        ...
        if (vertex0 == _leftVertex) {
            _clippedVertices.put(LR.LEFT, new PointH(x0, y0));
            _clippedVertices.put(LR.RIGHT, new PointH(x1, y1));
        } else {
            _clippedVertices.put(LR.RIGHT, new PointH(x0, y0));
            _clippedVertices.put(LR.LEFT, new PointH(x1, y1));
        }
    }
}