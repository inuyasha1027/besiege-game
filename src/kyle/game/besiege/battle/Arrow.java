...
public class Arrow extends Actor {
	...
	private final float ACCURACY_FACTOR = .02f;
...
//		setY(getY() + velocity.y);
		...
		pos_x_int = (int) (pos_x +.5);
		pos_y_int = (int) (pos_y +.5);
		
		// check for collision
		if (this.height < COLLISION_HEIGHT && pos_x_int < stage.size_x && pos_y_int < stage.size_y && pos_x_int > 0 && pos_y_int > 0 && stage.map[pos_y_int][pos_x_int] != null && stage.map[pos_y_int][pos_x_int].team != this.team)
			collision(stage.map[pos_y_int][pos_x_int]);
		
//		if (Math.abs(pos_x-this.dest_x) < THRESHOLD && Math.abs(pos_y-this.dest_y) < THRESHOLD)
//			this.stopped = true;
		if (height < -.1) this.stopped = true;
	}
...
//			setX(stage.scale * (temp_offset_x+stuck.prev_x + stuck.currentX() * stage.unit_width));
//			setY(stage.scale * (temp_offset_y+stuck.prev_y + stuck.currentY() * stage.unit_height));
//			
...
//		this.temp_offset_x = that.currentX() - this.pos_x;
//		this.temp_offset_y = that.currentY() - this.pos_y;
		this.destroy();
	}
